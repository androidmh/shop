  ...
  <div class="index-wrap">
    <div class="index-left">
      <div class="index-left-block">
        <h2>全部产品</h2>
        <template v-for="prodect in productList">
          <h3 :key="prodect.value">{{prodect.title}}</h3>
          <ul :key="prodect.value">
            <li v-for="item in prodect.list" :key="item.value">
              <a :href="item.url">{{ item.name }}</a>
              <span v-if="item.hot" class="hot-tag">HOT</span>
            </li>
          </ul>
          <div class="hr" :key="prodect.value" v-if="!prodect.last"></div>
        </template>
      </div>
